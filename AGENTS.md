...
**Atualização de estado — Onda 5 (2026-08-05):** Blueprint aprovado por DEC-51 e Etapa 8 local autorizada por DEC-52 para as fatias 029–035, sempre uma por vez com `$tdd`, pre-flight e Red Team de implementação. Fatia 029 (feature flag/pre-flight) e fatia 030 (`appointment_series`, extensão de `create_appointment`, RPCs de criação/edição/pausa/cancelamento) implementadas localmente na branch `feat/onda5-blueprint-and-fatia-029`, com materialização parcial + `appointment_series_conflicts` + retry idempotente (remediação forward-only de 2026-08-05, corrigindo o desvio inicial de tudo-ou-nada contra o Blueprint §3.1.4). 802/802 pgTAP, `supabase db lint`/`db advisors --local` sem achados, zero regressão. Camada Express/Jest para as RPCs de série continua deliberadamente adiada (decisão fechada por interview, 2026-08-04) — só SQL+pgTAP nesta fatia. `staging`, `main` e produção seguem não autorizados sob DEC-52.
