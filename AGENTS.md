...
`docs/KORTEXOS_5_1_2_MASTER_BRIEFING_CANONICO.md` é a visão de produto final vigente **e**, desde o encerramento do MVP, a única trilha ativa de execução (Trilha F do `docs/INDEX.md`). Ela não substitui as Invariantes acima nem autoriza domínio, tabela ou endpoint novo por si só — `docs/waves/KORTEXOS_5_1_2_TRUTH_MAP.md` audita a lacuna entre a fundação em produção e aquela visão, e a promoção segue etapas gated (Migration Map → Blueprint → SQL) que exigem aprovação explícita do Platform Owner em cada uma. Estado em 2026-07-28: Onda 0 está implementada em `staging`; a correção da Onda 1 é `PARCIAL` e `NO-GO` para produção; a Onda 2 teve Etapa 8 local regularizada por DEC-42, com migrations/testes verificados, mas segue sem autorização para `staging`/`main` até os gates de ambiente, entrega e homologação; a Onda 3 (Compensation — staff levels e comissão de venda) teve o Blueprint aprovado por DEC-46 e está **completa e mesclada em `staging`** (fatias 017-022, PR #24 e PR #25) — Etapa 8 e a promoção regularizadas por DEC-47; uma auditoria pós-merge achou 2 gaps reais (vínculo pacote↔pedido inexistente; `commission_sale_records` sem proteção de mutação), corrigidos por DEC-48 (emenda pontual à fronteira "`checkout_close` intocado") via fatias 021-022, verificados não só em pgTAP (661/661) mas na aplicação real rodando localmente (checkout com pacote, rejeição de comissão indevida, guard de imutabilidade, tudo exercitado via backend/PWA locais). Nenhum gap técnico conhecido em aberto na Onda 3 além da promoção formal. A Onda 4 (Calendar Policy/D02, Availability Resolver/D07, Resource Orchestration/D11) teve o Blueprint aprovado por DEC-49 (2026-07-29) — fundação sem ativação, `create_appointment`/`checkout_close` intocados; Red Team de desenho em 2 rodadas, a 2ª (agente independente) achou e corrigiu 6 gaps reais antes da aprovação (RLS vazando entre unidades, timezone ausente, concorrência otimista incompleta de `resource_locks`, FK sem amarração de `unit_id`, trigger de vigência sem validação de ordem, regra "turno ⊆ horário" sem materialização — ver ADR 0022). Etapa 8 aprovada pelo Platform Owner na mesma sessão — as 6 fatias (023-028) implementadas via `$tdd`: `calendar_policies`/`professional_shifts` (vigência versionada, turno ⊆ horário), `calendar_holidays`/`calendar_exceptions`/`calendar_time_off`, `calendar_policy_conflicts` (fila automática), `resources`/`resource_locks` (concorrência otimista completa + RPCs), as 3 funções do Resolver (timezone via `units.timezone`), e o endpoint Express `GET /units/:unitId/availability` (primeira rota desta trilha gated por feature flag real — corrigido de quebra um bug pré-existente em `requireFeatureFlag.js`, nunca antes conectado a nenhuma rota). Um Red Team de **implementação** pós-Etapa-8 (código real, disparado por relatório colado pelo usuário, reverificado achado a achado antes de aceitar — nenhum aceito por relato) achou 8 gaps reais que o Red Team de desenho não podia ver: bypass de unit scope na rota Express, elegibilidade tri-state (ADR 0010) ignorada, isenção `exceptional_opening` conceitualmente errada (removida, não corrigida — exceção de uma data não pode autorizar turno recorrente), timezone de feriado org-wide usando a unidade errada, segunda liberação de `resource_lock` aceita, FK de `calendar_exceptions` sem vínculo profissional↔unidade, `search_path` não fixado em 2 funções, e uma contradição documental na ADR 0022. Todos os 8 corrigidos (ver ADR 0022). Evidência final: 763/763 pgTAP, 319/319 testes de backend (`node --test`), sem regressão — só em ambiente local, nada commitado/mesclado em `staging` ainda. Produção segue em Fase 11, nenhuma migration da trilha 5.1.2 chegou lá; `kortex-api-staging` (Render) segue suspenso por billing (DEC-40), bloqueando homologação. Próxima sessão: revisar e commitar/PR a Onda 4, ou desbloquear infraestrutura de staging/homologação — nenhuma decisão de produto pendente na Onda 4.

**Atualização de estado — Onda 5 (2026-08-05):** Blueprint aprovado por DEC-51 e Etapa 8 local autorizada por DEC-52 para as fatias 029–035, sempre uma por vez com `$tdd`, pre-flight e Red Team de implementação. Fatia 029 (feature flag/pre-flight) e fatia 030 (`appointment_series`, extensão de `create_appointment`, RPCs de criação/edição/pausa/cancelamento, **camada Express das RPCs de série entregue** em `backend/src/modules/appointmentSeries/`) implementadas localmente na branch `feat/onda5-blueprint-and-fatia-029`, com materialização parcial + `appointment_series_conflicts` + retry idempotente (remediação forward-only de 2026-08-05, corrigindo o desvio inicial de tudo-ou-nada contra o Blueprint §3.1.4); issue 031 absorvida sem fatia própria.

Fatias 032–035 implementadas na sequência **033 → 032 → 034 → 035** (ordem de dependência real do Blueprint §6, não a numeração das issues: o trigger de titular de 033 precisa existir antes de `appointment_groups` de 032 poder dar titular automático a cada filho). Fatia 033: `appointment_participants`, trigger de titular (`AFTER INSERT` em `appointments`), backfill idempotente, `appointment_participant_add` e endurecimento de `update_appointment` — titular agora é imutável fora de um 6º parâmetro interno (`p_allow_client_transfer`, default `false`) que só `appointment_replan_with_hold` aciona; a assinatura antiga de 5 argumentos foi derrubada (`create or replace` não substitui aridade diferente) e a linha de titular migra junto no único caminho de transferência aprovado. Fatia 032: `appointment_groups`/`appointments.group_id`, criação atômica tudo-ou-nada de 2–10 filhos, `appointment_group_member_add`/`_update`/`_cancel`, status do grupo inteiramente derivado dos filhos reais via trigger de sincronização (nunca setado à mão). Fatia 034: `waitlist_entries`/`waitlist_entry_professionals`/`waitlist_offers`, matcher idempotente (`waitlist_matcher_run`) com token opaco hasheado, TTL/cooldown configuráveis, ausência de preferência = qualquer profissional elegível. Fatia 035: `waitlist_offer_accept/_decline/_expire`, CAS por `status='OFFERED'`, HOLDING puramente transacional (nunca observável pós-commit), ofertas irmãs da mesma onda viram `SUPERSEDED` ao vencer uma delas — canal de identidade de cliente (OTP/sessão) permanece lacuna conhecida, sem infraestrutura de auth client-facing nesta base de código; posse do token é o portão de autorização real por ora. Nenhuma das fatias 032-035 construiu contrato Jest/rota Express — leitura literal do Aceite escrito de cada issue (nenhuma pede), mesma disciplina de não expandir escopo.

Evidência: 862/862 pgTAP, 325/325 backend (`node --test`), `supabase db lint --local` sem achados, zero regressão nas fatias/Ondas anteriores (Red Team de implementação achou e corrigiu, antes deste veredito: ambiguidade de overload em `update_appointment` que quebrava `appointment_series_update`/`appointment_group_update`/`_cancel` e testes pré-existentes; `appointment_participants` com `on delete restrict` que travava um `DELETE` de fixture já testado — trocado para `cascade`; `private.onda5_waitlist_settings` chamada de dentro do matcher via sessão JWT inexistente em contexto `security definer` interno — resolvida lendo `organizations.settings` diretamente no matcher). `rpc_deposit_hold_financial_identity_test.sql` teve sua expectativa atualizada de `P0007` para `P0022`, documentando a supersessão do guard antigo (hold ativo) pelo novo guard geral de titular imutável. `staging`, `main` e produção seguem não autorizados sob DEC-52.
