...
| Onda 3 — Compensation | **REAL em `staging`, PARCIAL**; fatias 017-020 (Feature Flag, staff_levels, staff_level_service_overrides/resolve_service_pricing, comissão de venda) implementadas via `$tdd` (647/647 pgTAP) e mescladas em `staging` (PR #24), regularizado por DEC-47; auditoria pós-merge achou 2 gaps reais (vínculo pacote↔pedido inexistente; `commission_sale_records` sem proteção de mutação) — correção autorizada por DEC-48, fatias 021-022 **em andamento**. Produção segue em Fase 11 — nenhuma migration da trilha 5.1.2 chegou lá | `NO-GO` para `main`/produção até fatias 021-022 fecharem e os gates de ambiente/entrega/homologação passarem (bloqueados por DEC-40, `kortex-api-staging` suspenso) | [Onda 3](waves/onda-3-compensation/) |
