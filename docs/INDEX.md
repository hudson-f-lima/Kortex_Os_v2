...
| Onda 5 — Recurring, Group Booking & Waitlist | **REAL local** — Blueprint aprovado (DEC-51), Etapa 8 local autorizada (DEC-52). Fatias 029 (feature flag/pre-flight) e 030 (`appointment_series`, extensão de `create_appointment`, RPCs de série + camada Express) implementadas via `$tdd`; remediação forward-only de 2026-08-05 corrigiu um `NO-GO` de auditoria (materialização tudo-ou-nada → parcial + `appointment_series_conflicts` + retry), absorvendo integralmente o escopo da issue 031 (marcada `ABSORVIDA`, sem fatia própria). Fatias 033 (`appointment_participants`, trigger de titular, titular imutável em `update_appointment` via escape hatch controlado), 032 (`appointment_groups`, criação atômica 2–10 filhos, status derivado dos filhos), 034 (`waitlist_entries`/`_offers`, matcher idempotente) e 035 (aceitar/recusar/expirar oferta, CAS, HOLDING puramente transacional) implementadas via `$tdd`, na ordem de dependência real do Blueprint §6 (033 antes de 032). 862/862 pgTAP, 325/325 backend, `supabase db lint --local` sem achados, sem regressão | `NO-GO` para `staging`/`main`/produção sob DEC-52 (nenhuma fatia pendente na Onda 5) | [Onda 5](waves/onda-5-recurring-group-waitlist/) |
...
- [Onda 5 — Recurring, Group Booking & Waitlist](waves/onda-5-recurring-group-waitlist/) — Blueprint aprovado por DEC-51; fatias 029-035 implementadas localmente (issue 031 absorvida pela remediação de 030); nenhuma fatia pendente, promoção a `staging`/`main` segue não autorizada sob DEC-52.
