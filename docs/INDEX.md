...
| Onda 6 — Checkout final: reabertura de comanda | **REAL local — fatias 055–056 fechadas (DEC-66)**: schema de versão/reabertura, RLS/grants, fatos financeiros imutáveis e primitive privada de ledger/journal; a 056 preserva o grant administrativo, nega chamada direta por `authenticated`/`service_role` e restringe `reception` à própria unidade (27 pgTAP novos; regressão do ledger 33/33; lint sem erros). Fatias 057–061 continuam aprovadas para execução sequencial via TDD. | Não autoriza ativação, `staging`, `main` ou produção | [Onda 6](waves/onda-6-checkout-reopen/BLUEPRINT_ONDA_6.md) |
...
- [Onda 6 — Checkout final: reabertura de comanda](waves/onda-6-checkout-reopen/BLUEPRINT_ONDA_6.md) — Blueprint aprovado por DEC-62; fatias 055–056 implementadas e validadas localmente por Red Team (`GO`), com 057–061 aprovadas sob DEC-66; sem ativação ou promoção.
