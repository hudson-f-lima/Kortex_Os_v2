...
| Onda 6 — Checkout final: reabertura de comanda | **BLUEPRINT APROVADO (DEC-62)** — quatro rodadas de Red Team fecharam vínculo ledger↔pedido, trava de depósito, concorrência, rollback da flag e grants das primitives. | Etapa 8 bloqueada até aprovação do fatiamento e autorização própria | [Onda 6](waves/onda-6-checkout-reopen/BLUEPRINT_ONDA_6.md) |
| Onda 7 — Agenda: reagendamento por arraste e redimensionamento | **PARCIAL** — reagendamento por arraste (`starts_at`/`professional_id`) entregue (frontend-only, branch `feat/agenda-drag-and-resize-clean`); redimensionamento de duração com Migration Map v1.3 **aprovado** (DEC-64) | Etapa 7 (Blueprint) desbloqueada para as Ondas 0-6; **para a Onda 7, aguarda esta branch de reconciliação (DEC-65) ser mesclada em `staging`**; Etapa 8 bloqueada em qualquer caso | [Migration Map §3, Onda 7](waves/KORTEXOS_5_1_2_MIGRATION_MAP.md) |
...
> **Colisão de numeração (DEC-65):** as branches `codex/onda6-checkout-reopen` e `codex/speckit-runner-telemetry` registraram DEC-62 de forma independente e não coordenada; a numeração canônica (DEC-62 Onda 6, DEC-63 runner, DEC-64 Onda 7) está fixada em DEC-65, aprovada pelo Platform Owner e registrada em `docs/reconcile-dec62-onda6-vs-runner` (ainda não mesclada em `staging`). Nenhuma das duas branches originais deve mesclar em `staging` sem primeiro alinhar sua numeração interna a este registro.
...
- [Onda 6 — Checkout final: reabertura de comanda](waves/onda-6-checkout-reopen/BLUEPRINT_ONDA_6.md) — Blueprint aprovado por DEC-62 após Red Team de desenho `GO`; Etapa 8 depende de fatiamento aprovado e autorização própria.
- [Onda 7 — Agenda: reagendamento por arraste e redimensionamento](waves/KORTEXOS_5_1_2_MIGRATION_MAP.md#onda-7--agenda-reagendamento-por-arraste-e-redimensionamento-d07-completa-m01--aprovada-dec-64-2026-08-11) — Migration Map v1.3 aprovado por DEC-64; Blueprint bloqueado até a reconciliação de numeração (DEC-65, já aprovada) ser mesclada em `staging`.
