...
| Onda 6 — Checkout final: reabertura de comanda | **REAL em staging — schema aplicado, dark launch preservado (DEC-68)**: as 14 migrations versionadas foram aplicadas no Supabase de staging; schema de versão/reabertura, RLS/grants, fatos financeiros imutáveis, journal privado, `checkout_close` dark-launched e Commands existem. A flag `checkout_reopen_enabled` continua desligada. Evidência: reset limpo, 1.060/1.060 pgTAP antes e depois de 328/328 backend, CI verde e API staging `/health` 200. | `main`/produção e ativação permanecem bloqueados até homologação autenticada do fluxo financeiro e veredito final de Environment Guardian + Delivery Guardian | [Onda 6](waves/onda-6-checkout-reopen/BLUEPRINT_ONDA_6.md) |
...
- [Onda 6 — Checkout final: reabertura de comanda](waves/onda-6-checkout-reopen/BLUEPRINT_ONDA_6.md) — Blueprint aprovado por DEC-62; fatias 055–061 implementadas, Red Team local `GO` e schema aplicado em staging sob DEC-68; sem ativação e sem promoção para `main`/produção enquanto a homologação autenticada não ocorre.
