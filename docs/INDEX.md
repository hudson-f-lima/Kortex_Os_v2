...
| Onda 3 — Compensation | **REAL** (017-020 em `staging`, PR #24; 021-022 em `staging` local descartável, branch `fix/onda3-corretiva-vinculo-pacote-imutabilidade`, ainda não mesclados); as 6 fatias (Feature Flag, staff_levels, staff_level_service_overrides/resolve_service_pricing, comissão de venda, vínculo pacote↔pedido, imutabilidade) implementadas via `$tdd` (661/661 pgTAP, mais backend 301/301 e PWA 108/108 verificados nesta sessão), regularizadas por DEC-47/DEC-48 a partir de achados de auditoria pós-merge (P1/P2/P3, todos fechados). Produção segue em Fase 11 — nenhuma migration da trilha 5.1.2 chegou lá | `NO-GO` para `main`/produção; 021-022 aguardam PR + merge em `staging` antes disso, depois os gates de ambiente/entrega/homologação (bloqueados por DEC-40, `kortex-api-staging` suspenso) | [Onda 3](waves/onda-3-compensation/) |
