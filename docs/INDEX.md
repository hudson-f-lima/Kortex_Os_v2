...
| Onda 7 — Agenda: reagendamento por arraste e redimensionamento | **PARCIAL** — reagendamento por arraste (`starts_at`/`professional_id`) entregue e testado (frontend-only, branch `feat/agenda-drag-and-resize-clean`, empurrada, sem PR aberto); redimensionamento de duração com Migration Map v1.3 **aprovado** (DEC-64) | Etapa 7 (Blueprint) da Onda 7 **desbloqueada** — DEC-65 mesclado em `staging` (PR #44, `3c6327a`); Blueprint ainda não iniciado; Etapa 8 bloqueada em qualquer caso | [Handoff de continuidade](waves/onda-7-agenda-drag-resize/ONDA_7_CONTINUATION_HANDOFF.md) |
