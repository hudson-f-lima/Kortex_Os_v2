...
- [KORTEXOS_5_1_2_BLUEPRINT_ONDA_2_DRAFT.md](KORTEXOS_5_1_2_BLUEPRINT_ONDA_2_DRAFT.md) — **Blueprint aprovado (DEC-41), fatiado em `issues/012` a `issues/016`.** Desenho da Onda 2 (KortexFlow Ledger + Wallet & Current Accounts, D15/D16): `kortex_accounts`, `kortex_ledger_transactions`/`entries`, `kortex_account_balances`, `client_wallets`, `staff_current_accounts`, `benefit_obligations`, `payout_batches`. Fundação só, sem produtor real — `checkout_close` intocado nesta Onda, ativação fica pra uma Onda 2b futura. Etapa 8 (SQL) segue bloqueada até autorização própria do Platform Owner — o Fatiamento não é essa autorização.
- [adr/0019-onda2-kortexflow-ledger-fundacao.md](adr/0019-onda2-kortexflow-ledger-fundacao.md) — decisão arquitetural da Onda 2: fundação sem ativação, razão auxiliar por unidade com FK dupla tenant-safe, projeção em cascata de dois níveis, porta única de escrita sem `CONSTRAINT TRIGGER`.
