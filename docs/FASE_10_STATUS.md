...
## Decisões em aberto — status parcial (ADR 0008)

As duas primeiras haviam sido fechadas na sessão de implementação original **sem pesquisa de mercado** — violação direta da regra de ouro do projeto. Ver [`docs/adr/0008-capacidade-profissional-default-allow-e-preco-de-pacote.md`](adr/0008-capacidade-profissional-default-allow-e-preco-de-pacote.md) para o relato completo — o ADR passou por três revisões (citação errada → pesquisa rasa de documentação → testemunho de operação real) e uma das duas decisões segue genuinamente em aberto.

- **Nome final da tabela**: `professional_service_capabilities` (mantido, sem objeção — trivial, não exigia pesquisa). **Fechado.**
- **`price_override` aplica em pacotes ou só avulso?**: **Fechado — Zenoti confirma que o preço do pacote sempre vence o override individual**: "if gel polish is part of a complete nail package, guests booking the same service via the package will have to pay the standard price... even if the employee performs the service" ([Zenoti Help — Price scaling](https://help.zenoti.com/en/configuration/employee-configurations/price-scaling.html)). Nenhuma mudança de código necessária.
- **Profissional sem vínculo pode tudo ou nada?**: **REABERTO, não fechado.** A citação original a `PLANEJAMENTO_CALENDARIO_OPERACIONAL.md §10.3` estava errada (é sobre jornada, Fase 15) — isso segue corrigido. Mas a pesquisa por documentação de vendor (Zenoti/Fresha em default-allow, Square/Mindbody em default-deny, Booksy "configurável" segundo o help center) foi contestada por um testemunho de operação real de 5 anos com Booksy: a atribuição é uma ação ativa no cadastro do profissional (escolher grupos/serviços), não "tudo ligado por padrão", e diferenciação de skill entre profissionais é a prática comum vivida, não exceção. Documentação de vendor descreve o que o produto permite tecnicamente; operação real descreve o que o salão de fato configura — o segundo pesa mais. **Nenhum código mudou.** Decisão de escopo pendente: manter default-allow (risco aceito documentado no ADR) ou escopar uma feature nova de gate por habilidade (custo real: migration de semântica invertida, UX de atribuição em massa, validação em `appointments`, migração de dados para não bloquear profissionais já cadastrados).
