...
## Decisões em aberto — resolvidas em 2026-07-16 após pesquisa própria (ADR 0008)

As duas primeiras haviam sido fechadas na sessão de implementação original **sem pesquisa de mercado** — violação direta da regra de ouro do projeto. Corrigido numa segunda passada, ver [`docs/adr/0008-capacidade-profissional-default-allow-e-preco-de-pacote.md`](adr/0008-capacidade-profissional-default-allow-e-preco-de-pacote.md) para o relato completo da pesquisa.

- **Nome final da tabela**: `professional_service_capabilities` (mantido, sem objeção — trivial, não exigia pesquisa).
- **`price_override` aplica em pacotes ou só avulso?**: **Pesquisado — Zenoti confirma que o preço do pacote sempre vence o override individual**: "if gel polish is part of a complete nail package, guests booking the same service via the package will have to pay the standard price... even if the employee performs the service" ([Zenoti Help — Price scaling](https://help.zenoti.com/en/configuration/employee-configurations/price-scaling.html)). Nenhuma mudança de código necessária — `checkout_close` (Fase 5.1) já não consulta capabilities para nenhum propósito, avulso ou pacote (campo ainda não fiado ao checkout, fora do escopo declarado da Fase 10). Decisão fixa o contrato para quando esse wiring acontecer numa fase futura.
- **Profissional sem vínculo pode tudo ou nada?**: **A citação original a `PLANEJAMENTO_CALENDARIO_OPERACIONAL.md §10.3` estava errada** — aquela seção é sobre jornada de trabalho (Fase 15), não capacidade de serviço (Fase 10), uma conflação nunca verificada. A primeira pesquisa (4 fontes: Fresha/Cal.com/Calendly/Square) sugeriu um falso consenso em default-allow. Uma segunda passada — pedida explicitamente pelo usuário para triangular por 4 quadrantes (nacional×global, beautytech×fora-do-nicho) — revelou o beautytech genuinely dividido: **Zenoti** e **Fresha** em default-allow, **Square** e **Mindbody** em default-deny, **Booksy** com os dois modos configuráveis. A decisão final (manter default-allow, comportamento já implementado em `resolveDurationMinutes`) se apoia especificamente em **Zenoti** — o benchmark mais usado em todo o histórico de ADRs deste projeto, com arquitetura estruturalmente idêntica à tabela `professional_service_capabilities` (override associável a job/employee, ausência = vale para todos) — não em "consenso de mercado", que não existe de forma clara neste caso. O modo Square/Mindbody (restringir por atribuição, feature distinta de override de valores) fica registrado como candidato futuro, sem evidência hoje que justifique construí-lo.
