...
- **Profissional sem vínculo pode tudo ou nada?**: **A citação original a `PLANEJAMENTO_CALENDARIO_OPERACIONAL.md §10.3` estava errada** — aquela seção é sobre `professional_working_hours_template` (jornada de trabalho, Fase 15), não sobre capacidade de serviço (Fase 10); foi uma conflação entre duas fases distintas do planejamento, nunca verificada na primeira passada. Pesquisa própria, cruzando o nicho direto (Fresha: "new team members are assigned to all services by default") com SaaS de agendamento genérico fora do nicho (Cal.com: `assignAllTeamMembers: true`; Calendly: eventos Collective/Round Robin incluem todos os membros por padrão), confirma **default-allow/opt-out**, não default-deny — sem capability cadastrada, o profissional usa os valores padrão do serviço e pode ser agendado normalmente (comportamento já implementado em `resolveDurationMinutes`, coincidentemente correto). Square Appointments é a única fonte pesquisada com padrão oposto (default-deny), mas resolve um problema estruturalmente diferente (visibilidade binária de serviço, não override de preço/duração).
