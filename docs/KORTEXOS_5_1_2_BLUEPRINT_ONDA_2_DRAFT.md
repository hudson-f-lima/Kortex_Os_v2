...
**Status:** Decisões de desenho fechadas via `$grill-me` (11 rodadas, 2026-07-26). `$kortex-qa-redteam` (gate de desenho): 1ª rodada `NO-GO` com 3 achados (concorrência na criação de conta por entidade, validação de tenant ausente em `kortex_ledger_post`, autorização da RPC não especificada) — todos fechados com mecanismo concreto (`ON CONFLICT ... DO NOTHING`, checagem de `organization_id` por linha antes do `INSERT`, `actor_has_role` explícito). 2ª rodada `GO`. **Aguardando aprovação explícita do Platform Owner.** Etapa 8 (SQL) continua exigindo autorização própria, não incluída em nenhuma aprovação de Blueprint.
