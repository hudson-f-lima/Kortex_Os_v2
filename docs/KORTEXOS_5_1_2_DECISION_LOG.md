...
| DEC-20 a DEC-28 | Decisões de processo/auditoria da ordem de construção (§22.1): escopo e formato do benchmark, estratégia de benchmark por Tier, aprovação do Comparative Proposal, aprovação do Truth Map, aprovação do Migration Map, aprovação do adendo de cobertura D02, aprovação dos pontos cegos pré-Blueprint (Rodada 4), reconsideração da hierarquia Empresa→Unidade (Onda 0), finalização do escopo de unidade + itens operacionais consolidados | Cronológica, imutável |
| DEC-29 | Encerramento formal da construção do MVP técnico (Trilhas A–E, `docs/INDEX.md`) e transição da execução para a Trilha F (5.1.2) — reorganização documental (arquivamento em `docs/legacy/mvp-tecnico/`) | Cronológica, imutável |
