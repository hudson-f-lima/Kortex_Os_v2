...
**Atualizado:** 2026-07-13 (módulo agenda: appointments)
...
| Backend Express | `PARCIAL` | `backend/` materializado: `/health`, middleware JWT (JWKS, ES256), contexto de organização (`X-Organization-Id` + membership real) e módulos `clients`/`professionals`/`services`/`products`/`catalog`/`appointments` completos (CRUD + roles); 93 testes (unit/integration/contract), incluindo fluxos reais contra Supabase Auth/DB local. Checkout/caixa ainda não têm rota |
...
Materializar os módulos de negócio restantes no backend Express (checkout, caixa) chamando `checkout_close`/`inventory_adjust` via `service_role`, seguindo o mesmo padrão (validação pura + serviço + rota fina, roles espelhando RLS/RPC); depois iniciar a PWA modular.
