...
**Atualizado:** 2026-07-13 (módulo catálogo: services + products)
...
| Backend Express | `PARCIAL` | `backend/` materializado: `/health`, middleware JWT (JWKS, ES256), contexto de organização (`X-Organization-Id` + membership real) e módulos `clients`/`professionals`/`services`/`products`/`catalog` completos (CRUD + roles); 77 testes (unit/integration/contract), incluindo fluxos reais contra Supabase Auth/DB local. Agenda/checkout/caixa ainda não têm rota |
...
Materializar os módulos de negócio restantes no backend Express (agenda, checkout, caixa) seguindo o mesmo padrão (validação pura + serviço + rota fina, roles espelhando RLS, RPCs para mutações transacionais); depois iniciar a PWA modular.
