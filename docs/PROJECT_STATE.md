...
**Atualizado:** 2026-07-13 (módulo clientes)
...
| Backend Express | `PARCIAL` | `backend/` materializado: `/health`, middleware JWT (JWKS, ES256), contexto de organização (`X-Organization-Id` + membership real) e módulo `clients` completo (CRUD + roles); 31 testes (unit/integration/contract), incluindo fluxos reais contra Supabase Auth/DB local. Profissionais/catálogo/agenda/checkout/caixa ainda não têm rota |
...
Materializar os módulos de negócio restantes no backend Express (profissionais, catálogo, agenda, checkout, caixa) seguindo o padrão do módulo `clients` (validação pura + serviço + rota fina, roles espelhando RLS, RPCs para mutações transacionais); depois iniciar a PWA modular.
