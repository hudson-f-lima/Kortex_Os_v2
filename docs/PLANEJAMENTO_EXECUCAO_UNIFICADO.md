...
**Decisões em aberto — resolvidas em 2026-07-16 após pesquisa própria em duas passadas (ver ADR 0008)**: haviam sido fechadas na sessão de implementação sem pesquisa (violação da regra de ouro), corrigido retroativamente. Nome final da tabela: `professional_service_capabilities` (mantido, sem controvérsia). `price_override` em pacotes: **pesquisa em Zenoti confirma que o preço do pacote sempre vence o override individual do profissional**; `checkout_close` (Fase 5.1) já não consulta capabilities para nenhum propósito — decisão fixa o contrato para quando o wiring de preço acontecer numa fase futura. Profissional sem vínculo: a citação original a `PLANEJAMENTO_CALENDARIO_OPERACIONAL.md §10.3` era uma **conflação incorreta** (aquela seção é sobre jornada, Fase 15, não capacidade de serviço). Uma primeira pesquisa (Fresha/Cal.com/Calendly/Square) sugeriu falso consenso em default-allow; uma segunda passada, pedida pelo usuário para triangular por 4 quadrantes (nacional×global, nicho×fora-do-nicho), revelou o beautytech **genuinely dividido** (Zenoti/Fresha em default-allow, Square/Mindbody em default-deny, Booksy nos dois modos) — a decisão final (manter default-allow) se apoia em Zenoti especificamente, por ser o benchmark mais usado neste projeto e ter arquitetura estruturalmente idêntica à tabela já implementada, não em "consenso de mercado". O modo Square/Mindbody (restringir por atribuição) fica registrado como feature candidata futura, não descartado nem implementado sem evidência de necessidade.
