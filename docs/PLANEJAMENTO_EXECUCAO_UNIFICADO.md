...
**Decisões em aberto — resolvidas**: Nome final da tabela: `professional_service_capabilities` (mantido). `price_override` em pacotes: não resolvido nesta fase — `checkout_close` (Fase 5.1) segue resolvendo preço de pacote por alocação proporcional do avulso; capabilities de preço não foram fiadas ao checkout (fora do escopo declarado desta fase). Profissional sem vínculo: segue a recomendação de `PLANEJAMENTO_CALENDARIO_OPERACIONAL.md §10.3` — sem capability, cai no padrão do serviço; bloqueio de disponibilidade é escopo da Fase 15.
