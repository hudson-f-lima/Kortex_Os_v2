...
**Decisões em aberto (ver ADR 0008 — status parcial, não tratar como totalmente fechado)**: haviam sido fechadas na sessão de implementação sem pesquisa (violação da regra de ouro). Nome final da tabela: `professional_service_capabilities` (mantido, sem controvérsia — fechado). `price_override` em pacotes: **fechado** — Zenoti confirma que o preço do pacote sempre vence o override individual do profissional; `checkout_close` (Fase 5.1) já não consulta capabilities para nenhum propósito, decisão fixa o contrato para quando o wiring de preço acontecer numa fase futura. **Profissional sem vínculo pode ser agendado — REABERTO, não fechado.** A citação original a `PLANEJAMENTO_CALENDARIO_OPERACIONAL.md §10.3` era uma conflação incorreta (jornada, Fase 15, não capacidade de serviço) — isso segue corrigido. Mas a pesquisa por documentação (Zenoti/Fresha em default-allow, Square/Mindbody em default-deny, Booksy "configurável" segundo o help center) foi contestada por testemunho de operação real do usuário (5 anos em Booksy): a atribuição no Booksy real é uma ação ativa no cadastro (escolher grupos/serviços), não "tudo ligado por padrão", e diferenciação de skill entre profissionais é a prática comum, não exceção. **Decisão de escopo — RESOLVIDA (2026-07-16):** nem manter default-allow fixo nem forçar default-deny globalmente. [ADR 0010](adr/0010-elegibilidade-tri-state-fail-closed.md) (Accepted) especifica uma terceira via — tri-state (INHERIT/ENABLED/DISABLED) configurável por organização, com default `ENABLED` preservado para todo tenant existente (zero risco de bloqueio surpresa). Especificação completa em 4 ADRs, todos Accepted: [0010](adr/0010-elegibilidade-tri-state-fail-closed.md) (modelo tri-state), [0011](adr/0011-snapshot-operacional-agendamento.md) (snapshot de duração/elegibilidade), [0012](adr/0012-idempotencia-concorrencia-otimista-appointments.md) (RPC transacional + idempotência + `version`), [0013](adr/0013-change-plan-booking-settlement.md) (diff + confirmação). Migration estrutural (Phase 3) ainda não iniciada.
