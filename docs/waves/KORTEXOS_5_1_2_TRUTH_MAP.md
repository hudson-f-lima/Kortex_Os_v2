...
| M04 — reabertura de comanda (DEC-03/11) | `CRÍTICO` / `AUSENTE` | Só existe `order_refund`; `docs/architecture/adr/0007-cash-sessions-void-vs-refund.md` já registra o gap como decisão consciente para Fase 12 | DEC-03/11-13 exigem reversão em cascata (comissão, gorjeta, benefício, estoque, pagamento) + financial lock; estorno atual não é substituto |
...
| M05 — reversão de comissão, comissão de venda, payout | `CRÍTICO` / `AUSENTE` | `docs/architecture/adr/0005-reversao-de-comissao-em-estornos.md` registra intenção futura; sem payout batch, staff current account, clawback ou comissão de venda | Sem base para Gates 14/18 e DEC-15/18 |
...
5. **Release não é gateado.** Deploy do Pages não depende do sucesso da CI; a segunda superfície de deploy (Render) tem configuração incompatível (base path + CORS) nunca validada em uso real; a migration da Fase 11 segue sem confirmação de paridade em produção (`docs/legacy/mvp-tecnico/PROJECT_STATE.md`, "Próxima Ação Imediata" — confirmada em produção logo em seguida, ver o mesmo arquivo).
