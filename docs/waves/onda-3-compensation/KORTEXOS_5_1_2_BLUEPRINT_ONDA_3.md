...
**Status:** **RASCUNHO, aguardando `$kortex-qa-redteam` (gate de desenho) e aprovação explícita do Platform Owner.** Nenhuma linha de SQL, migration ou Etapa 8 autorizada por este documento — só contrato técnico.
**Etapa:** 7 (Blueprint), em redação.
...
Leitura direta de `supabase/migrations/20260713060000_professional_commissions_checkout.sql` confirma que `checkout_close` resolve preço de serviço lendo `services.price_cents` diretamente (linha 203-206 e 268-271) — **nunca** consulta `professional_service_capabilities.price_override_cents`/`duration_override_minutes`, apesar dessa tabela existir desde a Fase 10. O override de preço/duração por profissional×serviço já é um gap real e anterior a esta Onda, não uma regressão introduzida aqui. Isso é diretamente relevante à Decisão 2 (§3.2) e é registrado como achado, não corrigido nesta Onda — corrigir exigiria tocar `checkout_close`, fora do raio de explosão que este Blueprint aceita.
...
`returns table (commission_type text, commission_value bigint)`, `language sql stable security definer`. Lê `packages.sale_commission_type`/`sale_commission_value` direto (sem cascata — campo flat). `revoke all from public, anon, authenticated`.

### `commission_sale_records`
`id` uuid PK, `organization_id` uuid (FK `organizations`), `unit_id` uuid (FK composta `units`, denormalizado de `orders.unit_id` no momento da gravação), `order_id` uuid (FK composta `orders`), `package_id` uuid (FK composta `packages`), `professional_id` uuid (FK composta `professionals` — o vendedor), `commission_type` text (`check in ('percentage','fixed')`), `commission_value` bigint (`check >= 0`), `commission_cents` bigint not null (`check >= 0`, valor resolvido em centavos, mesma convenção `order_items.commission_cents`), `status` text not null default `'accrued'` (`check in ('accrued','clawed_back')`), `kortex_ledger_transaction_id` uuid nullable (FK composta `kortex_ledger_transactions`, preenchida só por onda futura de ativação), `created_at`/`updated_at`. **Unicidade:** `(organization_id, order_id, package_id, professional_id)` — evita duplicar comissão de venda para a mesma combinação venda×pacote×vendedor.

### `commission_sale_record_create` (RPC)
`public.commission_sale_record_create(p_organization_id uuid, p_actor_user_id uuid, p_idempotency_key text, p_order_id uuid, p_package_id uuid, p_seller_professional_id uuid) returns jsonb`, `language plpgsql security definer`. Valida `actor_has_role(owner/admin/manager/reception)`, valida `order` existe/pertence à org/está `closed`, valida `package`/`professional` existem e pertencem à org, resolve via `resolve_sale_commission`; se `commission_type is null`, retorna no-op (`{"skipped": true, "reason": "package has no sale commission configured"}`) sem inserir linha; senão calcula `commission_cents` (mesma fórmula percentage/fixed de `checkout_close`) e insere, reaproveitando `private.idempotency_keys`.
