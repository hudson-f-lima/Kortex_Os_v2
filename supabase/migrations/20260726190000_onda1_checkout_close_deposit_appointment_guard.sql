-- Red Team pós-merge (DEC-36): a reconciliação de depósito em checkout_close
-- (fatia 004, 20260726170000_onda1_checkout_close_deposit_reconciliation.sql)
-- validava a captura do deposit_hold só por organization_id + appointment_id
-- — nada ligava o appointment_id do payload ao cliente/serviço do checkout
-- que estava de fato sendo fechado. Um checkout com QUALQUER item, desde
...
-- ao pedido não relacionado — bug de dinheiro confirmado, não hipotético.
--
-- Correção isolada: antes de tentar o CAS, confirma que o appointment_id
-- informado (a) pertence ao mesmo client_id do checkout e (b) tem seu
-- service_id entre os itens de serviço deste mesmo pedido. Se não bater,
-- o comportamento é idêntico ao de "nenhum appointment_id informado" — a
-- reconciliação simplesmente não executa (mesmo caminho já usado pelo CAS
-- de corrida com a fatia 005), nunca um erro silencioso: se o chamador
-- esperava o desconto do depósito e ele não se aplica, a checagem
-- pré-existente `v_paid <> v_total` já rejeita o checkout de qualquer forma.
--
-- Corpo abaixo é cópia integral da versão vigente (fatia 004), com apenas a
-- nova guarda de client_id/service_id adicionada antes do UPDATE de CAS —
-- nenhuma outra linha de lógica é alterada.
