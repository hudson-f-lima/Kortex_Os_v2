...
-- emenda pontual ao Blueprint §1/§3.2 e à ADR 0020): fecha o achado P1 de
-- uma auditoria pós-merge (DEC-47) — hoje é estruturalmente impossível
-- saber, pelo banco, que pacote foi vendido em qual pedido.
-- `order_items.kind` só aceita 'service'/'product'; checkout_close dissolve
-- cada pacote em N linhas de serviço com preço rateado e descarta o
-- package_id (v_ref_id é sobrescrito na próxima iteração do loop).
--
-- Escopo estrito autorizado por DEC-48: coluna nova + checkout_close
-- redefinida só para gravar essa coluna no INSERT do ramo de pacote que já
-- existe hoje. Nenhuma mudança em preço, rateio por maior-resto, totais ou
-- reconciliação de depósito. create_appointment e resolve_commission() não
-- são tocados.
...
begin
  ...
  if exists (
    ...
    raise exception 'pre-flight check failed: public.order_items.package_id already exists';
  end if;
end $$;

-- Nenhum novo valor em order_items.kind — evita reabrir o CHECK
-- (kind in ('service','product')) e as constraints que dependem dele
-- (order_items_service_needs_professional, order_items_product_no_commission).
-- Linhas expandidas de pacote continuam kind='service'; package_id é só um
-- carimbo adicional de proveniência.
alter table public.order_items
  add column package_id uuid,
  add constraint order_items_package_fk foreign key (organization_id, package_id) references public.packages(organization_id, id) on delete restrict;

-- checkout_close redefinida a partir da versão vigente
-- (20260726190000_onda1_checkout_close_deposit_appointment_guard.sql, corpo
-- idêntico linha a linha) com uma única mudança: os 3 INSERTs em
-- order_items ganham a coluna package_id — null nos ramos 'product'/
-- 'service' avulso, v_ref_id (o id do pacote) no ramo 'package'. Nenhuma
-- outra linha de lógica (preço, rateio por maior-resto, totais,
-- reconciliação de depósito) é alterada.
...
declare
  ...
  -- Guarda de cruzamento (DEC-36): appointment_id só reconcilia se de fato
  -- pertencer a este checkout (mesmo cliente, serviço presente no pedido).
  v_appt_client_id uuid;
  v_appt_service_id uuid;
begin
  ...
  --
  -- Guarda de cruzamento (DEC-36): só tenta o CAS se o appointment_id
  -- informado realmente corresponde a ESTE checkout — mesmo client_id do
  -- pedido, e seu service_id presente entre os itens já inseridos acima.
  -- Sem essa guarda, qualquer appointment_id de terceiros com hold ativo
  -- capturava o valor e aplicava a um pedido não relacionado.
  if v_appointment_id is not null then
    select client_id, service_id into v_appt_client_id, v_appt_service_id
    from public.appointments
    where organization_id = p_organization_id and id = v_appointment_id;

    if v_appt_client_id is not null
      and v_appt_client_id = nullif(p_payload ->> 'client_id', '')::uuid
      and exists (
        select 1 from public.order_items
        where organization_id = p_organization_id and order_id = v_order_id
          and kind = 'service' and service_id = v_appt_service_id
      )
    then
